...
{
    public class CongestionTaxCalculatorBase
    {
        ...
        private static int GetTaxRate(ICollection<TaxRates> taxRates, int hour, int minute)
        {
            ...
            {
                if (taxRate.TimeFrom.Hour <= hour && taxRate.TimeTo.Hour >= hour 
                    && (taxRate.TimeFrom.Minutes >= minute || minute <= taxRate.TimeTo.Minutes))
                {
